...
def get_signed_request(request):
    ...
    try:
        if (request.session.get('signed_request') and request.method.upper() != 'POST') \
                or 'signed_request' not in request.POST:
...
class FacebookProtectionMixin(object):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.signed:
            return redirect(f'https://apps.facebook.com/{settings.FACEBOOK_APP_ID}')
